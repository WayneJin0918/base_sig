...
from cambrian.train.patch_metadata import patch_metadata
patch_metadata()
import os
...
def random_rotation(image: Image.Image, max_angle: float) -> Image.Image:
    ...
    angle = random.uniform(-max_angle*0.9, max_angle*0.9)
    ...
    for batch_idx, cur_input_ids in enumerate(input_ids):
        ...
        for i in range(len(image_token_indices) - 1):
            ...
            if i < len(image_token_indices) - 2:
                ...
                log_rank0(f"cur_im_attention_mask is {cur_im_attention_mask}.")
                ...
                if batch_idx % 2 == 1:
                    cur_im_attention_mask = torch.zeros_like(cur_im_attention_mask, device=cur_im_attention_mask.device)
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        #             cur_attention_mask_tmp = attention_mask[i].clone()
        #             cur_attention_mask_tmp[image_position+1:] = attention_mask[i, image_position:-1]
        #             image_position + image_token_len = k
        #             k=k-2
        #             log_rank0(f"k-2 is {k}.")
...
def train(INDEX, attn_implementation=None):
    ...
    if model_args.vision_tower_aux_list is not None:
        ...
        if training_args.tune_mm_projector:
            tune_modules = ['mm_projector']
            exclude_params = [
                "model.mm_projector_aux_0.0.bias", 
                "model.mm_projector_aux_0.0.weight", 
                "model.mm_projector_aux_0.2.bias", 
                "model.mm_projector_aux_0.2.weight", 
                "model.mm_projector_aux_0.3.bias", 
                "model.mm_projector_aux_0.3.weight"
            ]
            
            for name, param in model.named_parameters():
                if any(listed_name in name for listed_name in tune_modules) and name not in exclude_params:
                    print_rank0('tuning {}'.format(name))
                    param.requires_grad = True
